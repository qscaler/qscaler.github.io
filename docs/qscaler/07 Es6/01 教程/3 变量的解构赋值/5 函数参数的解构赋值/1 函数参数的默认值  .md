[https://zhuanlan.zhihu.com/p/491868475](https://zhuanlan.zhihu.com/p/491868475)


**函数参数的默认值   +  对象的解构赋值的默认值**  
**函数参数的默认值   +  对象的解构赋值的默认值**  
**函数参数的默认值   +  对象的解构赋值的默认值**  
**函数参数的默认值   +  对象的解构赋值的默认值**  
**函数参数的默认值   +  对象的解构赋值的默认值**  
...
length属性的含义是，该函数预期传入的参数个数。某个参数指定默认值以后，预期传入的参数个数就不包括这个参数了。同理，后文的 rest 参数也不会计入length属性指定了默认值以后，函数的length属性，将返回没有指定默认值的参数个数。也就是说，**指定了默认值后，length属性将失真**
...
另外，一个容易忽略的地方是，参数默认值不是传值的，而是每次都重新计算默认值表达式的值。也就是说，参数默认值是惰性求值的。
    ...
    x = 100;
    foo() // 101
上面代码中，参数p的默认值是x + 1。这时，每次调用函数foo，都会重新计算x + 1，而不是默认p等于 100。但是每次计算都是100，除非改变x的值
与解构赋值默认值结合使用
