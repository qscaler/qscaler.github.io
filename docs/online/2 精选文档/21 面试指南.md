...
关于 ES6/7 的考查内容还有很多，本小节就不逐一介绍了，如果想继续深入学习，可以在线看《[ES6入门<span><span class="sr-only">(opens new window)</span></span>](http://es6.ruanyifeng.com/)》。
...
![](https://s.poetries.work/gitee/2020/07/guide/1.png)
...
> 目前已经有一个获取 HTTP 请求更加方便的 API：`Fetch`，通过`Fetch`提供的`fetch()`这个全局函数方法可以很简单地发起异步请求，并且支持`Promise`的回调。但是 Fetch API 是比较新的 API，具体使用的时候还需要查查 [caniuse<span><span class="sr-only">(opens new window)</span></span>](https://caniuse.com/)，看下其浏览器兼容情况。
...
![](https://s.poetries.work/gitee/2020/07/guide/2.png)
...
![](https://s.poetries.work/gitee/2020/07/guide/3.png)
...
![](https://s.poetries.work/gitee/2020/07/guide/4.png)
...
![](https://s.poetries.work/gitee/2020/07/guide/5.png)
...
![](https://s.poetries.work/gitee/2020/07/guide/6.png)
...
![](https://s.poetries.work/gitee/2020/07/guide/7.png)
...
![](https://s.poetries.work/gitee/2020/07/guide/8.png)
...
![](https://s.poetries.work/gitee/2020/07/guide/9.png)

> 加上`float:left`之后：

![](https://s.poetries.work/gitee/2020/07/guide/10.png)
...
> float 的设计初衷是解决文字环绕图片的问题，后来误打误撞用于做布局，因此有许多不合适或者需要注意的地方，上文基本都讲到了需要的知识点。如果是刚开始接触 float 的同学，学完上面的基础知识之后，还应该做一些练习实战一下 —— 经典的“圣杯布局”和“双飞翼布局”。这里就不再展开讲了，网上资料非常多，例如[浅谈面试中常考的两种经典布局——圣杯与双飞翼<span><span class="sr-only">(opens new window)</span></span>](https://juejin.im/entry/5a8868cdf265da4e7e10c133?utm_source=gold_browser_extension)（此文的最后两张图清晰地展示了这两种布局）。
...
![](https://s.poetries.work/gitee/2020/07/guide/11.png)
...
![](https://s.poetries.work/gitee/2020/07/guide/12.png)
...
![](https://s.poetries.work/gitee/2020/07/guide/13.png)
...
![](https://s.poetries.work/gitee/2020/07/guide/14.png)

> 然后，我们把第三个`<p>`改为`position:absolute;`，看看会发生什么变化。

![](https://s.poetries.work/gitee/2020/07/guide/15.png)
...
![](https://s.poetries.work/gitee/2020/07/guide/16.png)

> `fixed` 元素的定位是相对于 `window` （或者 `iframe`）边界的，和其他元素没有关系。但是它具有破坏性，会导致其他元素位置的变化。

![](https://s.poetries.work/gitee/2020/07/guide/17.png)
...
![](https://s.poetries.work/gitee/2020/07/guide/18.png) ![](https://s.poetries.work/gitee/2020/07/guide/19.png)
...
![](https://s.poetries.work/gitee/2020/07/guide/20.png)
...
![](https://s.poetries.work/gitee/2020/07/guide/21.png)
...
![](https://s.poetries.work/gitee/2020/07/guide/22.png)
...
![](https://s.poetries.work/gitee/2020/07/guide/23.png)
...
![](https://s.poetries.work/gitee/2020/07/guide/24.png)
...
> 对于零宽断言的详细介绍可以阅读「[零宽断言<span><span class="sr-only">(opens new window)</span></span>](https://deerchao.net/tutorials/regex/regex.htm#lookaround)」这篇文章。
...
![](https://s.poetries.work/gitee/2020/07/guide/25.png)
...
> 关于浏览器整个流程，百度的多益大神有更加详细的文章，推荐阅读下：《[从输入 URL 到页面加载完成的过程中都发生了什么事情？<span><span class="sr-only">(opens new window)</span></span>](http://fex.baidu.com/blog/2014/05/what-happen/) 》。
...
*   前端最常用的 IDE 有 [Webstorm<span><span class="sr-only">(opens new window)</span></span>](https://www.jetbrains.com/webstorm/)、[Sublime<span><span class="sr-only">(opens new window)</span></span>](https://www.sublimetext.com/)、[Atom<span><span class="sr-only">(opens new window)</span></span>](https://atom.io/) 和 [VSCode<span><span class="sr-only">(opens new window)</span></span>](https://code.visualstudio.com/)，我们可以分别去它们的官网看一下。
...
*   你此前做过的项目一定要用过 Git，而且必须是命令行，如果没用过，你自己也得恶补一下。对 Git 的基本应用比较熟悉的同学，可以跳过这一部分了。macOS 自带 Git，Windows 需要安装 Git 客户端，去 [Git 官网<span><span class="sr-only">(opens new window)</span></span>](https://git-scm.com/download/win) 下载即可。
...
*   最早普及使用的构建工具是 [Grunt<span><span class="sr-only">(opens new window)</span></span>](http://www.gruntjs.net/) ，不久又被 [Gulp<span><span class="sr-only">(opens new window)</span></span>](https://www.gulpjs.com.cn/) 给追赶上。Gulp 因其简单的配置以及高效的性能而被大家所接受，也是笔者个人比较推荐的构建工具之一。如果你做一些简单的 JS 开发，可以考虑使用。
...
*   首先，你需要安装 Node.js，没有安装的可以去 [Node.js 官网<span><span class="sr-only">(opens new window)</span></span>](http://nodejs.cn/download/) 下载并安装。安装完成后运行如下命令来验证是否安装成功。
...
*   针对移动端的抓包工具，Mac 系统下推荐使用 Charles 这个工具，首先 [下载<span><span class="sr-only">(opens new window)</span></span>](https://www.charlesproxy.com/download/) 并安装，打开。Windows 系统推荐使用 [Fiddler<span><span class="sr-only">(opens new window)</span></span>](http://rj.baidu.com/soft/detail/10963.html)，下载安装打开。两者使用基本一致，下面以 Charles 为例介绍。
...
![](https://s.poetries.work/gitee/2020/07/guide/26.png)
...
> [如何在跳槽前客观地评估自己的身价？<span><span class="sr-only">(opens new window)</span></span>](https://www.zhihu.com/question/28313219/answer/331599918)
