...
<!--![](/assets/img/flags-mount.4756a068.png) -->
...
<!--![](/assets/img/vnodedata-style.ad61c1c6.png) -->
...
如上代码可知，我们为 `elementVnode` 添加了一个子节点，该子节点是一个边长为 `50px` 的绿色正方形，使用渲染器渲染修改后的 `elementVnode` 的效果如下：
 ...
 <!--![](/assets/img/vnodedata-children.0d6064f7.png) -->
...
如上高亮代码所示，我们给 `switch` 添加了一个 `case` 语句块，用来匹配 `VNodeData` 中的 `class` 数据，由于我们将 `data.class` 设计成了可直接使用的类名列表字符串，所以只需要直接将 `data.class` 赋值给 `el.className` 即可，如下是渲染 `elementVNode` 的效果：
 ...
 <!--![](/assets/img/mount-element-class.3873d5f4.png) -->
...
<!--![](/assets/img/mount-element-attr-prop.876ad25d.png) -->
...
其效果如下，当点击红色方块时会触发点击事件执行回调函数：
 ...
 <!--![](/assets/img/mount-element-event.36e70dcc.png) -->
...
使用渲染器渲染如上 `elementVnode` 的结果如下图所示：
 ...
 <!--![](/assets/img/mount-text.10e948e2.png) -->
...
最终的渲染效果如下图所示：
 ...
 <!--![](/assets/img/mount-fragment.82152fec.png) -->
...
使用渲染器渲染该 `elementVNode` 的效果图如下：
 ...
 <!--![](/assets/img/mount-portal.835104ff.png) -->
...
该组件的 `render` 函数返回了它要渲染的内容，如下是使用渲染器渲染后的效果：
 ...
 <!--![](/assets/img/mount-stateful-comp.75876604.png) -->
...
<!--![](/assets/img/mount-functional-comp.3208eb1c.png) -->
